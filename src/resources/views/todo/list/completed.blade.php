<x-app-layout>
    ...
    <x-alert />
    @if ($todos->isEmpty())
        <p class="text-gray-500 dark:text-gray-400">完了済みのタスクはありません</p>
    @else
        @foreach ($todos as $todo)
            <div class="mb-3 p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
                ...
                <p class="mb-3 font-normal text-gray-700 dark:text-gray-200">完了日時：{{ $todo->completed_at }}</p>
                <div class="flex justify-between">
                    <x-primary-button-link href="{{ route('todo.complete.undo', ['id' => $todo->id]) }}">
                        未完了にする
